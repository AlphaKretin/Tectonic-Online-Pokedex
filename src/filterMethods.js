...
function setupFilters() {
  ...
  speciesInput.addEventListener("keyup", function (event) {
    ...
    let option = selectedFilter.options.find(
      (x) => x[1].toLowerCase() === input
    );
    if (option) {
...
function buildDropdown(event) {
  let input = speciesInput.value.trim().toLowerCase();
  let options = selectedFilter.options.filter((x) =>
    x[1].toLowerCase().includes(input)
  );
  inputDropdown.innerHTML = "";
  for (let i = 0; i < options.length; i++) {
    let option = document.createElement("li");
...
function filterType(option) {
  let func = (x) =>
    species[x].type.primary === option[0] ||
    species[x].type.secondary === option[0];
...
function filterMove(option) {
  ...
  if (option === "RECALC") {
    ...
    for (const activeFilter of activeFilters) filterMove(activeFilter);
    return;
  }
  ...
  let func = function (x) {
    ...
    for (const key in learnset) {
      ...
      else found = learnset[key].find((y) => y === option[0]);
...
function filterMoveType(option) {
  ...
  if (option === "RECALC") {
    let activeFilters = [...filter.active];
    for (const activeFilter of activeFilters) filterMoveType(activeFilter);
    return;
  }
  ...
  let func = function (x) {
    ...
    for (const key in learnset) {
      if (key === "levelup" || key === "prevo")
        found = learnset.levelup.find(
          (y) => moves[y[0]].type === option[0] && moves[y[0]].power > 0
        );
      else
        found = learnset[key].find(
          (y) => moves[y].type === option[0] && moves[y].power > 0
        );
      if (found) return true;
    }
    return false;
...
function filterAbilities(option) {
  let func = (x) =>
    species[x].abilities.primary === option[0] ||
    species[x].abilities.secondary === option[0] ||
    species[x].abilities.hidden === option[0];
  addFilter(filters["Ability"], option, func);
}

function filterEggGroup(option) {
  let func = (x) =>
    species[x].family.eggGroup.primary === option[0] ||
    species[x].family.eggGroup.secondary === option[0];
  addFilter(filters["Egg Group"], option, func);
}

function filterHeldItem(option) {
  let func = (x) =>
    species[x].items != null &&
