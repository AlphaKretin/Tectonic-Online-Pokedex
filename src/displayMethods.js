...
function displaySpeciesRow(tracker, key) {
  ...
  for (const key in stats)
    currentRow.append(
      buildWrapperStat("td", "speciesStat", stats[key], mon.stats[key])
    );
}
...
function displaySpeciesPanel(mon) {
  ...
  for (let i = 0; i < Object.keys(stats).length - 1; i++) {
    ...
    statWrapper.append(
      buildWrapperStatFull("div", "infoStat", stats[stat], mon.stats[stat])
    );
  }
  statWrapper.append(
  ...
  for (const [ID, data] of tables) {
    ...
    if (data && data.length > 0) populateTable(ID, data);
    else table.classList.toggle("hide");
  }
  ...
  if (mon.family.variant)
    wrapper.append(
      buildWrapper(
        "div",
        className + "Region",
        regions[mon.family.region].variant
      )
    );
...
function buildWrapperAbilities(tag, className, a) {
  ...
  if (a.secondary)
    wrapper.append(
      buildWrapper("div", className + "Secondary", abilities[a.secondary].name)
    );

  if (a.hidden)
    wrapper.append(
      buildWrapper("div", className + "Hidden", abilities[a.hidden].name)
    );
  ...
  if (a.primary)
    wrapper.append(
      buildWrapper(
        "div",
        className + "Primary",
        abilities[a.primary].name + " - " + abilities[a.primary].description
      )
    );
  ...
  if (a.secondary)
    wrapper.append(
      buildWrapper(
        "div",
        className + "Secondary",
        abilities[a.secondary].name + " - " + abilities[a.secondary].description
      )
    );
  ...
  if (a.hidden)
    wrapper.append(
      buildWrapper(
        "div",
        className + "Hidden",
        abilities[a.hidden].name + " - " + abilities[a.hidden].description
      )
    );
...
function buildWrapperChangelog(tag, className, mon) {
  ...
  if (mon.changelog.type) {
    ...
    typeWrapper.append(
      buildWrapperTypes(
        "div",
        "infoChangelogOldType",
        types[mon.changelog.type.primary],
        types[mon.changelog.type.secondary]
      )
    );
    ...
    for (const ability of ["primary", "secondary", "hidden"]) {
      ...
      if (oldAbility && newAbility)
        abilityWrapper.append(
          buildWrapper(
            "div",
            "infoChangelogAbility " + ability,
            oldAbility + " → " + newAbility.name
          )
        );
      else if (newAbility)
        abilityWrapper.append(
  ...
  if (mon.changelog.stats) {
    ...
    for (const stat in mon.changelog.stats) {
      let statClass =
        mon.changelog.stats[stat] < mon.stats[stat]
          ? "infoChangelogBuff"
          : "infoChangelogNerf";
      statsWrapper.append(
        buildWrapper(
...
function buildWrapperFamilyTree(tag, className, mon) {
  ...
  if (mon.family.forms) {
    wrapper.append(
      buildWrapper("div", "infoTreeFormsLabel", "Alternate Forms")
    );
...
function buildWrapperCoverageDefensive(
  tag,
  className,
  primary,
  secondary = null
...
function buildWrapperTypeMatchup(key, matchup) {
  ...
  wrapper.append(
    buildWrapper(
      "div",
      "typeMatchupMultiplier x" + matchup * 100,
      matchup + "x"
    )
  );
...
function buildWrapperHeldItems(tag, className, i) {
  ...
  if (i.common)
    wrapper.append(
      buildWrapper("div", className, "Common: " + heldItems[i.common].name)
    );
  if (i.rare)
    wrapper.append(
      buildWrapper("div", className, "Rare: " + heldItems[i.rare].name)
    );
...
function buildBackgroundColor(currentRow, mon) {
  ...
  currentRow.style.backgroundImage =
    ...
    currentRow.style.backgroundColor.substr(3).replace(")", ", 0.4)") +
    ", rgb(63, 40, 40, 0.4))";
