...
function sortSpeciesRow(tracker, properties) {
  ...
  function tiebreaker(a, b) {
    ...
    comp = basicCompare(
      library[a].family.forms.indexOf(a),
      library[b].family.forms.indexOf(b)
    );
    return comp;
  }
  if (properties.by.length === 1) {
    ...
    compare = function (a, b) {
      let comp =
        library[a][property] == library[b][property]
          ? 0
          : library[a][property] < library[b][property]
          ? -1
          : 1;
    ...
    compare = function (a, b) {
      let comp =
        library[a][property][subproperty] == library[b][property][subproperty]
          ? 0
          : library[a][property][subproperty] <
            library[b][property][subproperty]
          ? -1
          : 1;
      return comp == 0 ? tiebreaker(a, b) : comp * factor;
    };
...
function sortLevelUpMovesRow(tracker, properties) {
  ...
  if (property === "level") {
    ...
    compare = function (a, b) {
      let comp =
        library[a[0]][property] == library[b[0]][property]
          ? 0
          : library[a[0]][property] < library[b[0]][property]
          ? -1
          : 1;
